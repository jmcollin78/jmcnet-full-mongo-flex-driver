package jmcnet.mongodb.messages.interpreter
{
	 ...
	 */
	public class GetLastErrorResponseInterpreter extends BasicResponseInterpreter
	{
		private static var log:JMCNetLog4JLogger = JMCNetLog4JLogger.getLogger(GetLastErrorResponseInterpreter);
		
		public function GetLastErrorResponseInterpreter()	{ }
			...
			if (rep.isOk) {
				if (rep.interpretedResponse.length == 1) {
					...
					if ((rep.interpretedResponse.hasOwnProperty("ok") && rep.interpretedResponse.ok == 0) || (rep.interpretedResponse.hasOwnProperty("errmsg") && rep.interpretedResponse.errmsg != null)) {
						rep.isOk = false;
						rep.errorMsg = "("+rep.interpretedResponse.code+")"+rep.interpretedResponse.err;
						log.warn("There is an err field -> mark the response as in error. ErrorMsg="+rep.errorMsg);
					}
				}
				else {
					rep.isOk = false;
					rep.errorMsg = "Error : GetLastError don't get one document in response but gets "+rep.interpretedResponse.length+" documents.";
				}
			}
